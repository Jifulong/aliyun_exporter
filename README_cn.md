...
`aliyun-exporter` 使用阿里云 SDK v2，支持三种认证方式：
...
credential:
  access_key_id: <YOUR_ACCESS_KEY_ID> # 选填，不填则走 RRSA/ECS RAM Role 等方式
  access_key_secret: <YOUR_ACCESS_KEY_SECRET> # 选填，需要和 access_key_id 一起填写
  region_id: <REGION_ID> # 默认值: 'cn-hangzhou'
  # RRSA（选填，仅在非 ACK 自动注入场景下需要）：
  # type: oidc_role_arn
  # role_arn: <ROLE_ARN>
...
本项目 fork 自 [aylei/aliyun-exporter](https://github.com/aylei/aliyun-exporter)，感谢原作者及各位贡献者的工作。